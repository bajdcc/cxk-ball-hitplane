(function() {
    $.getJSON("//gateway.baidu.com/acl/baidu.php", {},
        function(data, status) {
            //console.log(data);
            if (data["s"]) {
                if (data['c'] == 4) $("button").css('display', "block");
                //ready1();
            } else {
                //document.write("当前禁止访问");
                //
                layx.msg("当前禁止访问！");
                setTimeout(function() { window.location.href = "http://c.a.io" }, 1000);
            }
        });

    if (!ojstart) { $("button").css('display', "block"); return; }
    ctlist = [];
    $.getJSON("//j.learn.io/contestrank-api.php", { cid: ctid }, function(d) {

        ctlist = d;
        //console.log(ctlist);
        setTimeout(showInfoSt, 50);

    }).fail(function(jqXHR, textStatus, errorThrown) { console.error(textStatus) });

    var ss = 0;

    function showInfoSt() {

        if (!window._login || !_login['s']) { //layx.msg("登录中");
            ++ss;
            if (ss > 5) { $("button").css('display', "block"); return; }
            setTimeout(showInfoSt, 2000);
            return;
        }
        var login = _login.st || st;

        var suc = false;
        for (var ctt in ctlist) {
            var ct = ctlist[ctt];
            var cls = parseInt(ct.id.substr(2, 3));
            var cid = parseInt(ct.id.substr(5, 2));
            var cond = (cls == login.clsid && cid == login.sid && ct.total >= total);
            if (cond) {
        ...
        if (suc) {
            ...
            layx.msg("你的成绩已合格！<br>允许上网！");
        } else {

            layx.msg("你的成绩不合格！<br>你的成绩为：" + ct.total + "<br>要求为：" + total);
...
document.addEventListener("keyup", function(ev) {
    if (ev.key == 'a' || ev.keyCode == 37) {
        dd['a'] = false;
    } else if (ev.key == 'd' || ev.keyCode == 39) {
        dd['d'] = false;
    } else if (ev.key == 'w' || ev.keyCode == 38) {
        dd['w'] = false;
    } else if (ev.key == 's' || ev.keyCode == 40) {
        dd['s'] = false;
    }
});
...
function moving(d) {
    if (d['a']) {
        selfplan.planX -= 10;
        ourPlan.style.left = selfplan.planX + 'px';
    }
    if (d['d']) {
        selfplan.planX += 10;
        ourPlan.style.left = selfplan.planX + 'px';
    ...
    if (d['w']) {
        selfplan.planY -= 10;
        ourPlan.style.top = selfplan.planY + 'px';
    }
    if (d['s']) {
        selfplan.planY += 10;
        ourPlan.style.top = selfplan.planY + 'px';
